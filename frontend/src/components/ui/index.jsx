...
export function Badge({ children, variant = 'default', className = '' }) {
  const variants = {
    ...
    cake_shop:   'bg-pink-50 text-pink-700',
...
export function Button({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`inline-flex items-center gap-2 font-medium rounded-xl transition-all cursor-pointer
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          ...
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-lg
                       hover:bg-surface text-gray-400 hover:text-gray-600 transition-colors cursor-pointer"
          >
            <X size={16} />
          </button>
        </div>
...
export function EmptyState({ icon, title, description, action }) {
  return (
    <div className="flex flex-col items-center justify-center py-14 text-center">
      ...
      <p className="text-sm font-semibold text-gray-600">{title || 'Nothing here yet'}</p>
...
// ─────────────────────────────────────────────
// Spinner — fixed border width (border-[3px])
// ─────────────────────────────────────────────
...
export function Select({ label, error, children, className = '', ...props }) {
  return (
    <div className="flex flex-col gap-1">
      {label && <label className="text-xs font-medium text-gray-600">{label}</label>}
      <select
        className={`input appearance-none cursor-pointer ${error ? 'border-danger' : ''} ${className}`}
        {...props}
      >
        {children}
      </select>
